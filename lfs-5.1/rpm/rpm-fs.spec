...
Patch4          : rpm-4.0.4-obsolete-libio.patch
Patch5          : rpm-4.0.4-find-provides.patch
...
# turn off executable stack (needed for FC12)
if [ -x /usr/bin/execstack ]
then
    echo "clearing executable stack attribute on librpmio for FC12 SELinux"
    /usr/bin/execstack -c %{_prefix}/lib/librpmio-4.0.4.so
fi
